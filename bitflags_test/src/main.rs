...
// impl From<MyFlags> for JsFlags {
//     fn from(value: MyFlags) -> Self {
//         let flags : Vec<_> =  MyFlags::all().iter().collect();
//         for flag in  flags{
//             let js_value = format!("{flag}");
//             println!("js_value => {:?}", js_value);
//             println!("lippzhang => {:?}", flag);
//         }
//         value.name().into()
//     }
//   }
...
// }
fn main() {
    let flags: MyFlags = MyFlags::REQUIRE_SCOPE | MyFlags::EXPORT_STAR;

    if flags.contains(MyFlags::REQUIRE_SCOPE) {
        println!("Flag REQUIRE_SCOPE is set");
    }

    if flags.intersects(MyFlags::EXPORT_STAR | MyFlags::LOAD_CHUNK_WITH_MODULE) {
        println!("Flag EXPORT_STAR or LOAD_CHUNK_WITH_MODULE is set");
    }

    let combined_flags = MyFlags::REQUIRE_SCOPE | MyFlags::EXPORT_STAR | MyFlags::LOAD_CHUNK_WITH_MODULE;
    println!("Combined flags: {:?}", combined_flags);
    let flags : Vec<_> =  MyFlags::all().iter().collect();
    for flag in  flags{
        let js_value = format!("{flag}");
        println!("js_value => {:?}", js_value);
...
fn test_pretty_print() {
  let flags = MyFlags::PUBLIC_PATH;
  assert_eq!(format!("{flags}"), "__webpack_require__.p");
  let flags = MyFlags::GET_CHUNK_CSS_FILENAME;
